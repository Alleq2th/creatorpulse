...
// ─── APP-LIKE BACK NAVIGATION ───────────────────────────────────────────────
// By default a PWA has no concept of "back" — the hardware/gesture back button
// just closes the whole app. This gives it a real back stack: each meaningful
// screen change (tab switch, opening a sheet/modal, entering the camera) pushes
// a browser history entry with an "undo" action. Pressing back pops one step
// at a time through the app's own screens, and only exits once there's nothing
...
document.addEventListener("visibilitychange", () => { if(document.visibilityState === "hidden" && window.saveCaches) window.saveCaches(); });
window.addEventListener("pagehide", () => { if(window.saveCaches) window.saveCaches(); });
...
(function betaLayer(){
  ...
  window.fetch = async (input, init) => {
    ...
    if (isApi) {
      ...
      track("api_error", { url: url.split("?")[0], msg: lastRes ? ("HTTP "+lastRes.status) : String(lastErr).slice(0,80) });
    }
    // Return the real response when we have one, so callers can read the actual
    // error body instead of only ever seeing a generic "HTTP 502" with no reason.
  ...
  const openModal = async (kind) => {
    ...
    if (kind === "feedback") {
      ...
      box.querySelector("#cp-fb-send").onclick = async () => {
        ...
        } catch (e) { window._cpLastError = { msg: String(e && e.message).slice(0,200), at: Date.now() }; (window.cpModalNote||function(m){toast&&toast(m)})("Something went wrong — try again.", "err"); }
      };
    }
      ...
      box.querySelector("#cp-import").onchange = (ev) => {
        ...
        rd.onload = () => {
          try {
            ...
            modalNote("Imported ✓", "ok");
            setTimeout(closeModal, 700);
          } catch (e) { window._cpLastError = { msg: String(e && e.message).slice(0,200), at: Date.now() }; modalNote("Something went wrong — that file wasn't a valid backup. Try again.", "err"); }
        };
        rd.readAsText(f);
      };
    }
  };
  // Inline error/status card inside the open modal (replaces alert())
  const modalNote = (msg, kind) => {
    const box = document.getElementById("cp-modal-inner") || document.getElementById("cp-modal");
    if(!box){ if(window.toast) toast(msg); return; }
    let el = box.querySelector(".cp-inline-note");
    if(!el){ el = document.createElement("div"); el.className = "cp-inline-note"; box.prepend(el); }
    const err = kind !== "ok";
    el.style.cssText = `border:1px solid ${err?"#e6bcbc":"#bfe0c4"};background:${err?"#fff6f6":"#f4fbf5"};color:${err?"#a3423c":"#2f6b3c"};border-radius:10px;padding:10px 12px;margin:0 0 10px;font:600 12px system-ui;line-height:1.5`;
    el.textContent = msg;
    if(!err) setTimeout(()=>{ el && el.remove(); }, 2600);
  };
  window.cpModalNote = modalNote;
  ...
  window.cpExport = async () => {
    try {
      ...
      track("data_export");
    } catch(e){ window._cpLastError = { msg: String(e && e.message).slice(0,200), at: Date.now() }; modalNote("Something went wrong — try again.", "err"); }
  };
  ...
  window.renderContactModal = function(box){
    ...
    box.querySelector("#cp-ct-send").onclick = async () => {
      ...
      try {
        ...
        window._cpLastError = { msg: String(e && e.message).slice(0,200), at: Date.now() };
        (window.cpModalNote||function(m){ window.toast && window.toast(m); })("Something went wrong — try again.", "err");
      }
    };
  };
