...
function csApplyPreview(){
  try {
    ...
    stage.querySelectorAll('.cs-ed-vid').forEach(v => {
      ...
      // Only the active ("on") buffer should reflect the opacity slider — applying
      // it to the hidden preload buffer too was overriding its CSS opacity:0 with
      // an inline style, making the empty/not-yet-loaded buffer equally visible
      // and rendering as solid black on top of the real video.
      v.style.opacity = v.classList.contains('on') ? ((st.opacity==null?100:st.opacity)/100) : 0;
    });
