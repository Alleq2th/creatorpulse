    ...
    <div class="cs-ed-stage" id="cs-ed-stage" style="filter:${csFilterCss(st.filter)}">
      ...
      ${st.overlays.filter(o=>st.playhead>=o.startMs&&st.playhead<=o.endMs).map(o=>`
        <div class="cs-ed-overlay ${st.selectedId===o.id&&st.selectedType==='text'?'sel':''}"
          ...
          onclick="csSelectOverlay('${o.id}')" onpointerdown="csOverlayDragStart(event,'${o.id}')">${esc(o.text)}${st.selectedId===o.id&&st.selectedType==='text'?`<button class="ov-del" onclick="event.stopPropagation();csDeleteOverlay('${o.id}')">×</button>`:''}</div>
      `).join('')}
      ${(() => {
        const cap = (st.captions||[]).find(c => st.playhead >= (c.start||0)*1000 && st.playhead <= ((c.end!=null?c.end:(c.start||0)+2))*1000);
        if(!cap) return '';
        const cs = st.capStyle || (st.capStyle = {x:50,y:82,size:34,color:'#F5C518'});
        return `<div class="cs-ed-cap ${st.selectedType==='caption'?'sel':''}"
          style="left:${cs.x}%;top:${cs.y}%;font-size:${cs.size}px;color:${cs.color}"
          onclick="S.studio.selectedType='caption';S.studio.selectedId=null;render()"
          onpointerdown="csCaptionDragStart(event)">${esc(cap.text)}</div>`;
      })()}
...
function csPrompterScrollBind(){
  ...
  const pxPerSec = (S.studio.promptSpeed || 60) / 3; // promptSpeed is a 0-180-ish slider value → px/sec
...
window.csCaptionDragStart = (e) => {
  ...
  const startPctX = cs.x, startPctY = cs.y;
  let moved = false;
  capEl.setPointerCapture && capEl.setPointerCapture(e.pointerId);

  function onMove(ev) {
    const dx = ev.clientX - startX, dy = ev.clientY - startY;
    if (Math.abs(dx) > 2 || Math.abs(dy) > 2) moved = true;
    let nx = startPctX + (dx / rect.width) * 100;
    let ny = startPctY + (dy / rect.height) * 100;
    nx = Math.max(5, Math.min(95, nx));
    ny = Math.max(5, Math.min(95, ny));
    capEl.style.left = nx + "%";
    capEl.style.top = ny + "%";
    cs._pendingX = nx; cs._pendingY = ny;
  }
  function onUp() {
    document.removeEventListener("pointermove", onMove);
    document.removeEventListener("pointerup", onUp);
    if (moved && cs._pendingX != null) {
      cs.x = cs._pendingX; cs.y = cs._pendingY;
      delete cs._pendingX; delete cs._pendingY;
      render();
    } else if (!moved) {
      S.studio.selectedType = 'caption'; S.studio.selectedId = null; render();
    }
  }
...
window.csOverlayDragStart = (e, id) => {
  e.stopPropagation();
  const stage = document.getElementById("cs-ed-stage");
  ...
  const rect = stage.getBoundingClientRect();
  const startX = e.clientX, startY = e.clientY;
  const startPctX = o.x, startPctY = o.y;
  let moved = false;
  ...
  function onMove(ev) {
    ...
    overlayEl.style.left = nx + "%";
    overlayEl.style.top = ny + "%";
    o._pendingX = nx; o._pendingY = ny;
  }
  function onUp() {
    document.removeEventListener("pointermove", onMove);
    document.removeEventListener("pointerup", onUp);
    if (moved && o._pendingX != null) {
      o.x = o._pendingX; o.y = o._pendingY;
      delete o._pendingX; delete o._pendingY;
      csPushHistory && csPushHistory();
      render();
    } else if (!moved) {
...
window.csSplit = () => {
  ...
  if(!c){
    // Fall back to whichever clip is currently loaded/playing instead of failing silently.
...
window.csEditOverlayField = (k, v) => {
  ...
  o[k] = v;
  if(k === 'text'){
    // Update the live overlay + textarea directly — avoid a full render() per keystroke, which caused flicker/cursor jumps.
    const el = document.querySelector(`.cs-ed-overlay[data-ovid="${o.id}"]`);
    if(el){
      const del = el.querySelector('.ov-del');
      el.textContent = v;
    ...
    await new Promise(async (resolve) => {
      ...
      const draw = () => {
        ...
        for(const cap of (captions||[])){
          ...
          if(gMs>=gs && gMs<=ge){ csDrawCaption(ctx, cap.text||"", W, H, capStyle); }
        }
        requestAnimationFrame(draw);
      };
      v.play().catch(()=>{});
...
function csDrawCaption(ctx, text, W, H, capStyle){
  const cs = capStyle || {x:50,y:82,size:34,color:'#F5C518'};
  const size = Math.round((cs.size||34) * (W/720));
  ctx.font = `800 ${size}px "Inter", sans-serif`;
  ctx.textAlign = "center"; ctx.textBaseline = "middle";
  const cx = W*(cs.x/100), cy = H*(cs.y/100);
  const maxW = W*0.7;
  // Wrap onto multiple lines instead of overflowing/cutting off, matching the live preview.
