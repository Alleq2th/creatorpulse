...
//
// Two backgrounds are supported per slide:
//   - a real photo (from the story's Twitter/blog source image), cover-fit
//     and darkened with a scrim so text stays legible
//   - a full-bleed gradient fallback, used whenever no photo is available
//
// Fonts: "PT Serif" (headlines/stat numbers — matches the reference look:
// bold serif numerals, editorial feel) and "Inter" (body/labels, clean sans
// for readability at small sizes). Bundled in assets/fonts and registered
// with fontconfig by scripts/setupFonts.js at build/boot time. If those
// fonts aren't installed for any reason, these font-family names just fall
// back to the platform default, so this never hard-fails.
const sharp = require("sharp");
const fetch = require("node-fetch");

const FONT_DISPLAY = "'PT Serif', Georgia, serif"; // headline + stat numbers
...
const FORMATS = {
  instagram: { w: 1080, h: 1350, statSize: 220, statY: 480, statLabelY: 530, titleYWithStat: 620, titleYNoStat: 340, titleSize: 58, titleLine: 66, bodySize: 32, bodyLine: 44, titleChars: 20, bodyChars: 34, hookTitleSize: 88, hookTitleLine: 98, hookTitleChars: 13 },
  tiktok:    { w: 1080, h: 1920, statSize: 240, statY: 620, statLabelY: 675, titleYWithStat: 800, titleYNoStat: 480, titleSize: 60, titleLine: 70, bodySize: 34, bodyLine: 48, titleChars: 18, bodyChars: 32, hookTitleSize: 96, hookTitleLine: 106, hookTitleChars: 12 }
};
...
// Full-card gradient palettes — used as a fallback background when there's
// no real photo to work with. Each also carries an accent color (stat
// number / pill / swipe cue) and text colors chosen for contrast.
const PALETTE = {
...
function fitStatFontSize(text, maxWidth, baseSize) {
  const estCharWidth = 0.62; // PT Serif Bold digits, roughly
...
// Legibility scrim — used only when compositing over a real photo. Darkens
// top-to-bottom so header labels and body text both stay readable
// regardless of what's happening in the photo underneath.
function scrimDefs(w, h) {
...
// Background layer: either the gradient fill, or (when onPhoto=true) just
// the scrim, since the actual photo gets composited in underneath by
// renderStatCard before the text layer is placed on top.
function backgroundLayer(p, f, onPhoto) {
  if (onPhoto) return `<defs>${scrimDefs(f.w, f.h)}</defs><rect width="${f.w}" height="${f.h}" fill="url(#scrim)"/>`;
...
function renderHookSVG(p, f, slide, niche, slideNum, totalSlides, onPhoto) {
  ...
  <svg width="${f.w}" height="${f.h}" viewBox="0 0 ${f.w} ${f.h}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="72" y="${f.h-40}" width="${f.w-144}" height="4" fill="${p.accent}" opacity="0.6"/>
  </svg>`;
}

// ── Body slide: stat number + headline + supporting copy.
function renderBodySVG(p, f, slide, niche, slideNum, totalSlides, onPhoto) {
  ...
  <svg width="${f.w}" height="${f.h}" viewBox="0 0 ${f.w} ${f.h}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="72" y="${f.h-90}" width="${f.w-144}" height="4" fill="${p.accent}" opacity="0.6"/>
  </svg>`;
}

// ── Outro slide: the closing beat — a CTA, centered vertically, with an
// optional handle/tag line underneath. No stat, no clutter.
function renderOutroSVG(p, f, slide, niche, slideNum, totalSlides, onPhoto) {
...
// Fetches a background photo and returns it cover-cropped to exactly the
// card dimensions, ready to composite under the text/scrim layer. Returns
// null on any failure (missing URL, bad response, corrupt image) so the
// caller can fall back to the gradient — a broken photo should never take
// the whole card generation down with it.
async function fetchCoverImage(imageUrl, w, h) {
  ...
  try {
    ...
    return await sharp(buf).resize(w, h, { fit: "cover", position: "attention" }).jpeg({ quality: 90 }).toBuffer();
  } catch (e) {
    return null;
  }
}

async function renderStatCard(slide, niche, palette, format, imageUrl) {
  ...
  // Only the hook slide gets the real photo (the "cover" of the story) —
  // body/outro slides stay on the clean gradient so the stat numbers and
  // CTA text stay maximally legible.
  const cover = role === "hook" ? await fetchCoverImage(imageUrl, f.w, f.h) : null;
  const onPhoto = !!cover;

  const svg = role === "hook" ? renderHookSVG(p, f, slide, niche, slide.slideNum, slide.totalSlides, onPhoto)
    : role === "outro" ? renderOutroSVG(p, f, slide, niche, slide.slideNum, slide.totalSlides, onPhoto)
    : renderBodySVG(p, f, slide, niche, slide.slideNum, slide.totalSlides, onPhoto);

  const textLayer = await sharp(Buffer.from(svg)).png().toBuffer();
  if (!cover) return textLayer;

  return sharp(cover).composite([{ input: textLayer }]).png().toBuffer();
}

async function renderCardSet(slides, niche, palette, format, imageUrl) {
  const total = slides.length;
  const buffers = await Promise.all(
    slides.map((s, i) => renderStatCard({ ...s, slideNum: i + 1, totalSlides: total }, niche, palette, format, imageUrl))
  );
