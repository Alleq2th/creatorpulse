...
// ── HOOKS LIBRARY (20 reusable hooks per niche) ─────────────────────────────
const HOOK_TEMPLATES = [
  "Nobody's talking about this {topic} story, but they should be.",
  "If you create {topic} content and you're not doing this, you're leaving views on the table.",
  "I watched 50 {topic} videos so you don't have to. Here's what actually works.",
  "The {topic} trend that broke the algorithm this week.",
  "Stop scrolling. This {topic} take will change how you post.",
  "Three {topic} mistakes killing your reach right now.",
  "This is why your {topic} posts aren't hitting.",
  "The {topic} secret every top creator uses but never talks about.",
  "You've been thinking about {topic} wrong. Here's the fix.",
  "What no one tells you about going viral in {topic}.",
  "I tested this {topic} hook for 30 days. The results are wild.",
  "The one {topic} format that outperforms everything else in 2026.",
  "If I had to start a {topic} account today, this is exactly what I'd do.",
  "This {topic} moment is about to be everywhere. Post before it's dead.",
  "The unfair advantage in {topic} nobody's using.",
  "Why {topic} creators under 10K are winning right now.",
  "Steal this {topic} script. It's already going viral.",
  "The {topic} story the algorithm can't stop pushing.",
  "Here's the {topic} take that got me 2M views last week.",
  "Watch this before you post about {topic} again."
];
app.get("/api/hooks", (req, res) => {
  const niche = (req.query.niche || "your niche").toString();
  const topic = niche.replace(/\s*[\/\(\)].*$/,"").toLowerCase();
  res.json({ hooks: HOOK_TEMPLATES.map((h,i)=>({ id:i, text: h.replace(/\{topic\}/g, topic) })) });
});
