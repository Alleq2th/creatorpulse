  ...
  try {
      ...
      if (data.articles && data.articles.length > 0) {
        ...
        const scored = data.articles
          .map(a => {
            ...
            const descMatch = queryWords.filter(w => descLower.includes(w)).length >= 2;
            if (!titleMatch && !descMatch) return null;
            const matchCount = queryWords.filter(w => titleLower.includes(w)).length +
                              queryWords.filter(w => descLower.includes(w)).length * 0.5;
    ...
    const filteredItems = (feed.items || [])
      .filter(item => {
        ...
        const snippetMatch = queryWords.filter(w => snippet.includes(w)).length >= 2;
...
app.get("/api/blog-feed", async (req, res) => {
  ...
  res.json({ articles: allArticles.slice(0, 8) });
});
...
app.post("/api/generate-image", async (req, res) => {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      // If rate limited, fallback to FLUX.1-schnell (usually faster)
      if (r.status === 429) {
...
app.get("/api/google-oauth-callback", async (req, res) => {
  ...
  // We need the user's token to associate – pass it as a query param from frontend
  const { token } = req.query;
  try {
...
// ── FUTURE EVENTS ROUTE (for notifications) ──────────────────────────────────
  ...
  for (const feedUrl of feeds.slice(0, 2)) {
    try {
      ...
      const items = (feed.items || []).slice(0, 3).map((item) => {
        // Generate a plausible future date (1-30 days ahead)
    ...
    } catch (e) { console.log("Event feed error:", e.message); }
  }
