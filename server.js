...
app.get("/api/news", async (req, res) => {
  ...
  try {
    // Try NewsAPI first — use relevancy sort + quoted phrase-style AND logic for tighter matches
    if (NEWS_KEY && NEWS_KEY !== "YOUR_NEWS_KEY_HERE") {
      ...
      if (data.articles && data.articles.length > 0) {
        const niceQuery = query.toLowerCase();
        const queryWords = niceQuery.split(" ").filter(w => w.length > 3);

        // Score articles by how many query keywords actually appear in title — filters out loose matches
        const scored = data.articles.map(a => {
          const titleLower = (a.title || "").toLowerCase();
          const matchCount = queryWords.filter(w => titleLower.includes(w)).length;
          return { ...a, _matchCount: matchCount };
        }).filter(a => a._matchCount > 0) // must match at least one real keyword in the TITLE, not just anywhere
...
// ── ROUTE: IMAGE PROXY (avoids CORS issues drawing external article photos to canvas) ──
