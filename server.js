...
// ── 🆕 BLOG RSS FEEDS PER NICHE (additional sources with images) ────────────
...
app.get("/api/news", async (req, res) => {
  ...
  try {
    // ✅ FIX: sortBy=publishedAt for breaking news
...
// ── ROUTE: BLOG FEED (fetch from blog RSS) ─────────────────────────────────
app.get("/api/blog-feed", async (req, res) => {
  ...
  for (const feedUrl of feeds.slice(0, 3)) {
    try {
      ...
      console.log(`Error fetching blog feed ${feedUrl}:`, e.message);
    }
  }
...
// ── 🐦 TWITTER BREAKING NEWS ROUTE (existing) ──────────────────────────────
...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "openai/gpt-oss-120b", // ✅ confirmed working
        messages: [
...
// ── ROUTE: SAVE POST TO DATABASE (for calendar) ─────────────────────────────
